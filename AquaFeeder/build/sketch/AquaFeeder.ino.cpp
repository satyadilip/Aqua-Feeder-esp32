#include <Arduino.h>
#line 1 "C:\\Users\\nsaty\\Documents\\Dilip Work\\Aqua_feeder\\AquaFeeder\\AquaFeeder.ino"
// ============================================================================
...
void setup() {
    ...
    if (loraMgr.begin()) {
        ...
        if (config.loraOTAA) {
            if (loraMgr.join(config.loraDevEUI, config.loraAppEUI, config.loraAppKey)) {
                ...
                Serial.println("[LORA] OTAA Join SUCCESS!");
            } else {
                sysStatus.loraStatus = ConnStatus::ERROR;
                Serial.println("[LORA] OTAA Join failed — will retry later.");
            }
        } else {
            Serial.println("[LORA] No keys configured — configure via Web Dashboard.");
        }
    } else {
    ...
    cloudMgr.setLoRa(loraMgr.isAvailable() ? &loraMgr : nullptr);
